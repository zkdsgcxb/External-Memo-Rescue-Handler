...
2026-09-25：Guard 已接入 `DM_MPATH_PROBE_PATHS`，当前只维护和验收本机 **7.0.0-34** 基线，已删除旧内核兼容与降级分支。代码和故障试验仍限于 QEMU；源码提交不部署真实根盘自动保护。
...
- `guest/dm_monitor.py` 的 `DeviceMapper.target_version()` 在启动时通过已加载的 libdevmapper 查询内核 multipath target 版本，不启动外部命令；要求至少 1.15.0，否则直接拒绝启动。它是依赖检查，不是版本适配器。
...
删除兼容分支后，25 项实验室单元测试和 7.0 的 13 项直接 ABI 检查通过。完整 Ubuntu/LVM 根卷在 Git 克隆中同端口连续三次 `gap=0` 拔插，三次均实际调用内核接口后完成路径复核；233 次成功写入、0 错误，原进程/服务、直接块读取、数据前缀审计及 Git 完整性检查通过。最长观测写入约 1.043 秒，非保证上限。

本次构建、运行源码与报告哈希见 [当前基线验收摘要](results/2026-09-25-kernel7-only.json)。本次没有运行旧内核适配实验；下面的旧记录不作为当前支持范围。
...
当前单元测试覆盖异步阻塞、单任务上限、错误拒绝、启动依赖检查、generation/实例变化、健康期不读盘、过期后迟到结果不可复活，以及 resume 自身跨过期限。它们不会访问真实块设备；原来验证成功降级的测试已删除。
...
```bash
python3 lab/kernel_probe_test.py --build-dir lab/work/kernel-probe/current-only
python3 lab/auto_run.py --build-dir lab/work/kernel-probe/current-only --guest ubuntu --workload git-clone --cycles 3 --same-port --gap 0
python3 lab/auto_run.py --build-dir lab/work/kernel-probe/current-only --reconnect wrong --queue-seconds 4
python3 lab/measure_guard.py --build-dir lab/work/kernel-probe/current-only
