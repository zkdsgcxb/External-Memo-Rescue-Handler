...
**当前状态：已启动 7.0.0-34 并通过初步检查；随后按用户要求清退旧 6.8。** 当前不再提供 6.8 启动回退项，后续内核更新跟随 HWE 元包。日志对比见 [首次启动记录](HOST-POSTBOOT.md)，清退结果见文末。下文安装及首启准备部分保留当时记录。
...
此节为安装时的准备记录；下述 6.8 回退入口已在后续清退中移除。
...
用户在新内核启动并反馈暂时正常后，明确要求清退旧内核。重新确认正在运行 `7.0.0-34-generic`，模拟并锁定删除清单，再由本机管理员认证执行十个包的精确 purge：旧 6.8 镜像、模块、额外模块、两份头文件、两份专用工具，以及 `linux-generic`、`linux-image-generic`、`linux-headers-generic` 三个 GA 跟踪元包。

保留 HWE 元包和完整的 7.0 image/modules/headers/tools，后续更新继续沿官方 HWE 路线。`linux-libc-dev` 和 `linux-tools-common` 属于通用开发接口/工具，版本号含 6.8 不代表还留着一套可启动的旧内核，因此没有删除。未运行广泛的 `autoremove`。

清退后检查：十个包均已移除，`/boot` 旧内核文件及旧模块目录已消失，GRUB 无 6.8 项且语法检查通过；7.0 镜像和 initramfs 的 SHA256 保持不变，HWE 包完整，`dpkg --audit` 无异常。外置 EFI 文件和固件启动项保持一致。系统仍运行 7.0，没有额外重启。

Ubuntu 包钩子将 `vmlinuz.old` 和 `initrd.img.old` 改为指向当前 7.0；这两个名字现在不代表独立备用内核。历史 QEMU 镜像和研究证据仍留在 `lab/work/`，不属于宿主已安装内核。

删除包的元数据合计约 303 MB，另移除了约 83 MB 的旧 initramfs；这是元数据和文件长度口径，不是空闲空间的精确差值。完整脱敏记录和私有日志哈希见 [host-retirement-result.json](host-retirement-result.json)。
